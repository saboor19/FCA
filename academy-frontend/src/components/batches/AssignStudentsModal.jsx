...
export default function AssignStudentsModal({
  ...
  useEffect(() => {
    if (open) {
      fetchStudents();
      setSelectedStudents([]); // Reset selection on open
    }
  }, [open]);
            ...
            <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <h3 className="text-lg font-medium text-foreground">No Students Available</h3>
                  <p className="text-slate-500 dark:text-slate-400 mt-1 max-w-sm">
                    All students are currently assigned, or there are no student profiles matching this criteria.
                  </p>
                </div>
              ) : (
                  ...
                  {students.map((student) => {
                    ...
                    return (
                      <div
                        ...
                        className={`group flex items-center justify-between p-4 rounded-xl border transition-all cursor-pointer ${isSelected ? "border-slate-900 dark:border-slate-100 bg-slate-50 dark:bg-slate-800/50 ring-1 ring-slate-900 dark:ring-slate-100" : "border-border-custom bg-transparent hover:border-slate-300 dark:hover:border-slate-600 hover:bg-slate-50/50 dark:hover:bg-slate-800/30"}`}
                      >
                        ...
                        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${isSelected ? "bg-slate-900 border-slate-900 dark:bg-slate-100 dark:border-slate-100 text-white dark:text-slate-900" : "border-slate-300 dark:border-slate-600 group-hover:border-slate-400"}`}>
                          <AnimatePresence>
                            {isSelected && (
                              <motion.div
