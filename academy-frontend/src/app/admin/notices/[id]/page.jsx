    ...
    <DashboardLayout role="ADMIN">
        ...
        <div className="bg-card border border-border-custom rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="p-6 md:p-8 border-b border-border-custom bg-slate-50/50 dark:bg-slate-900/20">
            <div className="flex flex-wrap items-center gap-2 mb-4">
              ...
              <span
                className={`px-2.5 py-1 rounded text-[11px] font-bold uppercase tracking-wider ${getTypeClasses(
                  notice.type
                )}`}
              >
                {notice.type || "GENERAL"}
              ...
              <span
                className={`flex items-center gap-1 px-2.5 py-1 rounded border text-[11px] font-bold uppercase tracking-wider ${priorityConfig.classes}`}
              >
                {priorityConfig.icon}
          ...
          {/* Notice Body — RICH TEXT RENDERED AS HTML */}
          <div className="p-6 md:p-8">
            <h3 className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3">
              Message Content
            </h3>
            {/* 
              KEY CHANGE: Use dangerouslySetInnerHTML instead of plain text.
              The prose classes handle styling (headings, lists, links, etc.).
              Remove whitespace-pre-wrap since HTML handles its own spacing.
            */}
            <div
              className="prose dark:prose-invert max-w-none text-slate-700 dark:text-slate-300 leading-relaxed
                ...
                prose-blockquote:border-l-4 prose-blockquote:border-indigo-500 prose-blockquote:bg-slate-50 dark:prose-blockquote:bg-slate-800/50 prose-blockquote:pl-4 prose-blockquote:italic
                ...
                prose-pre:bg-slate-100 dark:prose-pre:bg-slate-800 prose-pre:rounded-xl prose-pre:p-4"
              dangerouslySetInnerHTML={{ __html: notice.description || "<p>No content provided.</p>" }}
            />
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 gap-y-6 gap-x-4">
              ...
              <div>
                ...
                <button
                  ...
                  className="font-medium text-foreground flex items-center gap-2 hover:text-indigo-600 transition"
                >
                  <Eye size={16} className="text-emerald-500" />
                  {notice.views || 0} Views <ArrowUpRightFromSquare size={14} />
                </button>
              </div>
      ...
      {showReaders && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="w-full max-w-2xl bg-card border border-border-custom rounded-3xl shadow-2xl overflow-hidden">
            {/* HEADER */}
            <div className="px-6 py-5 border-b border-border-custom flex items-center justify-between">
              ...
              <button
                ...
                className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-muted-foreground hover:text-foreground transition-colors"
              >
                <X size={20} />
              </button>
            </div>
            ...
            {/* BODY */}
            <div className="max-h-[500px] overflow-y-auto divide-y divide-border-custom">
              {notice.readBy?.length > 0 ? (
                notice.readBy.map((item, index) => (
                  <div
                    key={index}
